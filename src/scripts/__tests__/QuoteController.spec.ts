...
describe('Quote Controller', () => {
  describe('initialization', () => {
    it ('should be initialized properly with default parameters', () => {
      ...
      expect(controller._settings.nonNerdyOnly).toBe(false)
    })
    it('should be initializable with option to not show nerdy quotes', () => {
      const dao = new MockQuoteDAO()
      const factory = new MockQuoteFactory()
      const renderer = new MockQuoteRenderer()

      const controller = new QuoteController({ dao, factory, renderer, settings: {
        nonNerdyOnly: true
      }})

      expect(controller._settings.nonNerdyOnly).toBe(true)
    })
  })
  describe('update', () => {
    it('should order creation of a default quote and update its last id', async () => {
      ...
      factory.createQuote = jest.fn(
        () => new QuoteDTO(quoteData)
      )
